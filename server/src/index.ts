...
[modelsBaseDir, uploadDir].forEach((d) => {
...
async function downloadAndPrepare() {
  ...
  while (true) {
    ...
    process.stdout.write(
      `\r   ${bar} ${pct}%  ${formatBytes(downloadedBytes)} / ${formatBytes(totalBytes)}  ${speed} MB/s   `,
    );
  }
...
app.use((req, res, next) => {
  ...
  res.json = function (body: any) {
    console.log(
      `[${timestamp()}] ── RESPONSE #${reqId} (Status: ${res.statusCode}, ${Date.now() - start}ms) ─────`,
    );
    if (body?.text)
      console.log(
        `  Result:   "${body.text.substring(0, 100)}${body.text.length > 100 ? "..." : ""}"`,
      );
...
app.post(
  "/transcribe",
  express.raw({ type: "audio/wav", limit: "100mb" }),
  async (req, res) => {
